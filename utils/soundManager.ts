...
// Mock sound interface for fallback compatibility
interface MockSound {
  play: () => Promise<void>;
  pause: () => Promise<void>;
  stop: () => Promise<void>;
  ...
  private async initializeSounds() {
    try {
      ...
      for (const soundType of soundTypes) {
        try {
          // Create a mock sound object that provides the interface
          // In a real implementation, you would load actual sound files here
          const mockSound: MockSound = {
            play: async () => {
              console.log(`Playing sound: ${soundType}`);
              // Future: Add actual sound playback using expo-audio
            },
            pause: async () => {
      ...
      console.log('Sound manager initialized for SDK 53');
    } catch (error) {
